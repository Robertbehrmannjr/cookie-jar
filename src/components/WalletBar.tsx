...
import { LAMPORTS_PER_COOK } from '../lib/constants'
  ...
  useEffect(() => {
    ...
    const load = () =>
      connection
        ...
        .then((lamports) => !cancelled && setBalance(lamports / LAMPORTS_PER_COOK))
        .catch(() => {})
    load()
      ...
      <div className="wallet-bar-right">
        {connected && publicKey && (
          <div className="wallet-info">
            ...
            <span className="wallet-balance">{balance !== null ? `${balance.toFixed(4)} COOK` : '—'}</span>
          </div>
        )}
        <WalletMultiButton />
      </div>
